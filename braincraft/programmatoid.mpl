...
prgm_functions := {
  ...
  If_v = ((c, v1, v0) ->
    if nargs = 3 then Prod_b(H_(c), v1, (1 - H_(c)), v0)
 ...
 Prod_b = (() ->
     if type(nargs, odd) then Prod_b(args, 0)
     else sum(omega * h(args[2*k]/omega + omega * (H_(args[2*k-1]) - 1)), k = 1 .. nargs/2)
     fi),
 Softmax = (proc()
   local k,
     g := args[nargs],
  ...
  Delay_0 = (proc(o, i_1, i_0, T)
      local v := new_symbol("v"), s := new_symbol("s"), g := if T > 0 then 1 - 2^(-1/T) else 1 fi:
      s = If_b(And(s = 0, i_1 = 1), 1, And(o = 1, i_0 = 0), 0, o)
      v = (1 - g) * v + g * s,
      o = If_b(v > 0.5, 1, 0)
     end),
  Delay_1 = (proc(o, i_1, i_0, T)
      local v := new_symbol("v"), s := new_symbol("s"), g := if T > 0 then 1 - 2^(-1/T) else 1 fi:
      s = If_b(And(s = 0, i_1 = 1), 1, And(o = 1, i_0 = 0), 0, o)
      v = (1 - g) * v + g * s,
      o = If_b(v > 0.5, 1, 0)
...
prgm_compile := proc(prgm_file :: string)
  ...
  try 
    ...
    if prgm [networking] then  prgm [mollification] := true fi:
     ...
     (proc(eqs)
       ...
       (proc(eqs)
          local v := convert(ListTools[FindRepetitions](map(eq -> if type(eq, `=`) then op(1, eq) fi, eqs)), set):
       	  alert(nops(v) > 0, "The input has redundant left-hand-size variables: ", v)
       end)(eqs):
       map(proc(l, eqs)
         local eq := eqs[l]:
	 if (type(eq, `=`)) then
  ...
  if prgm[python] then 
    (proc(prgm)
      local c, wl:
      #### Avoids warning on unknown functions
      wl := interface(warnlevel=0):
      #### Builds the python source file
      c := cat(
        "# This file is automatically generated by the programmatoid compiler, better not edit\n",
	"import numpy as np\n",
        (if prgm[networking] then 
	   ("from programmatoid_challenge import h, NetworkState\n\nclass MyState(NetworkState):\n",
	    "\tdef init(self):\n",
	    #### Inits evaluation parameters
	    "\t\tself.data.update(dict({",op(map((n, prgm) -> cat("\"",n,"\": ", subs({true = True, false = False}, prgm[n]), ", ") , [challenge, timeout, runs, display], prgm)), " \"init\": True}))\n",
	    "\t\t",
	    #### Inits the network weights
	    StringTools[RegSubs]("cg" = "self.data[\"network\]",
	    StringTools[RegSubs]("[]][\t\n ]*$" = "",
	    StringTools[RegSubs]("= [[]" = "= ",
	      CodeGeneration[Python]([prgm[network][W_in], prgm[network][W], prgm[network][w], prgm[network][W_out]], defaulttype=float, output=string)))), "\n",
	    #### Inits the NetworkState remainder
	    "\t\tsuper().init(self)\n")
	 else
	   ("from programmatoid_challenge import h, H, Id, State\n\nclass MyState(State):\n",
	    "\tdef init(self):\n",
	    #### Inits evaluation parameters
	    "\t\tself.data.update(dict({", op(map((n, prgm) -> cat("\"", n, "\": ", subs({true = True, false = False}, prgm[n]), ", "), [challenge, timeout, runs, display], prgm)), " \"init\": True}))\n",
	    #### Inits the state variables
	    "\t\tself.data.update(dict({", op(map(n -> cat("\"", n, "\": 0, "), indets(prgm[flattened], name))), " \"init\": True}))\n",
	    "\n",
	    "\tdef update(self):\n",
	     ...
	     StringTools[RegSubs]("^" = "\t\t",
	     StringTools[RegSubs]("\n(.)" = "\n\t\t\\1",
	       CodeGeneration[Python](subs(map(n -> n = self_data[convert(n, string)], indets(prgm[flattened], name)), prgm[flattened]), defaulttype=float, output=string)))))
	  fi)):
  ...
  ## Stops the compilation if errors
    ...
    print(cat("\tcompilation-error: \"", lastexception,"\"")):
     ...
     if assigned(prgm[network]) then
       network = [  ### Outputs network meta-data
         ...
         op(map((n, prgm) -> n = StringTools[RegSubs]("cg.*= numpy.mat\\(([^)]*)\\)\n" = "\\1", CodeGeneration[Python](prgm[network][n], defaulttype=float, output=string)),
           [W_in, W, w, W_out], prgm))] else network = false fi ### Outputs network buffers
    ]), style=block))))))) end)(prgm)
end:
...
test := true:
if test then
  FileTools[Text][WriteFile]("/tmp/prgm_test.mpl", "[ prgm_options = { omega = 10, challenge = 0 },  Delay(b, i_t, 10),  a = H(H(b))]"):
