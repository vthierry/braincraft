\documentclass[a4]{article} \usepackage[left=2cm,right=3cm,top=1cm,bottom=2cm]{geometry}
...
Let us consider the following digital experimental setup, as described in the \href{https://github.com/rougier/braincraft/blob/master/README.md#introduction}{braincraft challenge presentation}.

\subsection{Simplified problem statement}

\centerline{\includegraphics[width=0.9\textwidth]{../../data/debug.png}}

As shown in this figure\footnote{Shared here by courtesy of Nicolas P. Rougier.}, \href{https://github.com/rougier/braincraft}{the braincraft challenge} bot moves at a constant with sensor inputs and one orientation output, it uses some energy and refill this energy on a given yellow location. The 2D space size is $[0, 1] \times [0, 1]$. The bot starts in the middle and is oriented upward.
\eqline{\begin{array}{|ll|l|}
  \hline\multicolumn{3}{|c|}{\mbox{{\bf Input variables}}}\\\hline
  p_l \in ]0_{\mbox{no-wall}}, 1_{\mbox{wall-hit}}] && \mbox{\parbox{5cm}{~\\Leftward wall proximity, at +30° on the left.\\}} \\\hline 
  p_r \in ]0_{\mbox{no-wall}}, 1_{\mbox{wall-hit}}] && \mbox{\parbox{5cm}{~\\Rightward wall proximity, at -30° on the  right.\\}} \\\hline 
  p_a \in ]0_{\mbox{no-wall}}, 1_{\mbox{wall-hit}}] && \mbox{\parbox{5cm}{~\\Ahead wall proximity, at 0°.\\}} \\\hline 
...
\\ \tab - three leftward, ahead, and rightward external sensors only are considered,
\\ \tab - color is input as binary variables, and always available,
\\ \tab - the output stands on positive normalized signal,
\\ \tab - the wall hit indicator is not used, and wall hits are ignored,
\\ \tab - the heuristics consider the energy source as constant and unbounded.

Neglecting the energy source bound, leak and refill mechanism, and ignoring wall hits yields sub-optimal heuristics, but the goal is not to ``win'' but the challeng, but to implement programmatoid and neuronoid mechanisms.

\subsection{Preprocessing}

\subsubsection{Proximity sensors}\begin{description}
\item[Motivation] Simplifies the navigation by using a simple triplet of input.
\item[Implementation] ~
...
\item[Implementation] For the distributed implementation, each camera color index value is mapped on each color channel input with the 0 value if the color is different and the 1 value if equal, e.g., using step unit:
  \eqline{c_{\dagger\bullet} \leftarrow \mbox{ if } \mbox{Or}_{k \in K_\dagger} (i_{\bullet} - c[k]) = 0 \mbox{ then } 1 \mbox{ else } 0, \dagger \in \{l_{\mbox{left}}, r_{\mbox{right}}\}, \bullet \in \{b_{\mbox{blue}}, r_{\mbox{red}}\}}
...
\subsubsection{Energy increase or decrease}\begin{description}
\item[Motivation] In order to detect if the bot has been reefed, we must save the energy value at different moment, and compare the values, because among the trajectory the absolute value varies.
\item[Implementation] It uses two variables
\eqline{\begin{array}{lll}
  g_{e1} \in [0, 1], &\left.g_{e1}\right|_{t = 0} = 0 & \mbox{Energy after the last turn.} \\
  g_{d1} \in [0, 1], &\left.g_{ed}\right|_{t = 0} = 0 & \mbox{Energy difference, before the next turn, and after the last turn.} \\
\end{array}}
which are updated during the navigation as detailled in the next section.
\end{description}

\subsubsection{Output unsigned normalization}\begin{description}
...
\\\tab (i) ahead by default, thanks to a linear correction, high enough to correct the direction, small enough to avoid oscillations, and either
\\\tab (ii) performs a quarter-turn in the preferred orientation as soon as a wall is closed ahead, or
\\\tab (ii) performs a about-turn if the energy has been increased by an energy source.

With this about-turn heuristic, the bot remains in the corridor with feeding source. However, to perform an about-turn the bot must hit the wall, with generates an energy loss, immedtaly compensated by refilling energy on the source.  This however does not take into account the source energy is bounded and requires time to refill.
...
\begin{figure}[H]
  \includegraphics[width=0.5\textwidth,valign=c]{./navigation.png}
    \hfill \begin{minipage}{\dimexpr 0.5\linewidth-\columnsep}
      \caption{Illustrates the navigation implementation. The navigation is made of quarter-turns to find the source and then about-turns to remain around the source:
        \\ \tab Quarter-turn is triggered by forward wall proximity and stops when this proximity is low enough. It is used to find the feeding source.
        \\ \tab About-turn is triggered by forward wall proximity and stops after a fixed delay. It is used to remain around the feeding source.}
        \label{navigation}
    \end{minipage}
\end{figure}

\item[Internal variables] \eqline{\begin{array}{lll}
  b_d \in \{0_{\mbox{rightward}}, 1_{\mbox{leftward}}\}, &\left.b_d\right|_{t = 0} = 0 & \mbox{Initial quarter-turn direction.} \\
  b_t \in \{0_{\mbox{forward}}, 1_{\mbox{turning}}\}, &\left.b_t\right|_{t = 0} = 0 & \mbox{Forward versus turning mode.} \\
  b_a \in \{0_{\mbox{quarter-turn}}, 1_{\mbox{about-turn}}\}, &\left.b_t\right|_{t = 0} = 0 & \mbox{Quarter-turn versus about-turn mode.} \\
\end{array}}

\item[Internal constants]  \eqline{\begin{array}{rcll}
    \gamma &=& 0.035 & \mbox{\parbox{12cm}{Linear feedback gain in forward mode, maximize stability without oscillation.}} \\
    \alpha    &=& 5.0 & \mbox{\parbox{12cm}{Maximal turn angle, allowing to perform a quarter turn in $18$ time steps.}} \\
    \beta_1   &=& 0.8 & \mbox{\parbox{12cm}{Proximity threshold triggering a quater-turn.}} \\
    \beta_0   &=& 0.7 & \mbox{\parbox{12cm}{Proximity threshold ending a quater-turn.}} \\
    \delta    &=& 36 & \mbox{\parbox{12cm}{About-turn duration in number of time steps.}} \\
\end{array}}
  
\item[Programmatoid solution]:\eqline{\begin{array}{rccc}
    d_l &\leftarrow& \mbox{if } b_t = 0 \mbox{ then } \gamma \, p_r  &  \mbox{elif } b_d = 1 \mbox{ then } \alpha \mbox{ else } 0 \\
    d_r &\leftarrow& \mbox{if } b_t = 0 \mbox{ then } \gamma \, p_l   &  \mbox{elif } b_d = 0 \mbox{ then } \alpha \mbox{ else } 0 \\
    &                 & \mbox{\small \begin{tabular}{c}linear correction to \\ maintain direction ahead\end{tabular}} & \mbox{\small \begin{tabular}{c}turning \\ left or right\end{tabular}} \end{array}}
 while the bot turning state implementation writes:
 \eqline{\begin{array}{rcl}
     \multicolumn{3}{l}{\mbox{Wall proximity turn trigerring, if not yet done:}} \\
     b_1 &=& p_a > 0.8 \mbox{ and } \mbox{ not } b_t \\
     \multicolumn{3}{l}{\mbox{About-turn choice iff the energy increased:}} \\
     b_a &\leftarrow& \mbox{if }  b_1  \mbox{ and } g_{e1} > 0 \mbox{ then } g_e > g_{e1} \mbox{ else } b_a \\
     \multicolumn{3}{l}{\mbox{About-turn direction change:}} \\
     b_d &\leftarrow& b_1 \mbox{ and } b_a \mbox{ then } 1 - b_d \mbox{ else } b_d \\
     \multicolumn{3}{l}{\mbox{About-turn delay, triggered at about-turn start:}} \\
    &&\mbox{Delay\_0}(b_w, b_1 \mbox{ and } b_a, \delta) \\
    \multicolumn{3}{l}{\mbox{Turn end triggering:}} \\
    b_0 &=& b_t \mbox{ and } \mbox{ if } b_a \mbox{ then } b_w \mbox{ else } p_a <  \beta_0 \\
   \multicolumn{3}{l}{\mbox{Turn mode detection:}} \\
    b_t &\leftarrow& \mbox{ if } b_1 \mbox{ then } 1 \mbox{ elif } b_0 \mbox{ then } 0 \mbox{ else } b_t\\
    \multicolumn{3}{l}{\mbox{Turn end detection:}} \\
    b_s &=& b_0 \mbox{ and not } b_1 \\
    \multicolumn{3}{l}{\mbox{Gauge energy update:}} \\
   g_{d1} &\leftarrow&  \mbox{ if } b_s \mbox{ then } g_e - g_{e1} \mbox{ else } g_{d1} \\
   g_{e1} &\leftarrow&  \mbox{ if } b_s \mbox{ then } g_e \mbox{ else } g_{e1} \\
\end{array}}
\end{description}
...
\\- As detailed in the sequel, the $\mbox{Delay\_0}(b_w, b_{\mbox{trigger}}, \delta)$ construct defines a delay of $\delta$ time steps, triggered by $b_{\mbox{trigger}}$ and which output $b_w$ equals $0$ during this delay.

\subsubsection{Task 1: Simple decision} \begin{description}
  \item[Heuristic]  Finds on which side is the energy loop and stay around the energy source until, when found.
\end{description}
...
\item[Heuristic] Restricts navigation to the open side with an energy source, as indicated by a blue color at the beggining.
    \item[Internal variable] \eqline{\begin{array}{lll}
        b_l \in \{0, 1\}  & \left. b_l \right|_{t = 0} = 0 & \mbox{Blue detected on the left.} \\
        b_r \in \{0, 1\}  & \left. b_r \right|_{t = 0} = 0 & \mbox{Blue detected on the right.} \\
    \end{array}}
    \item[Programmatoid solution] \eqline{\begin{array}{rcl}
        \multicolumn{3}{l}{\mbox{Detects, leftward or righward, the 1st blue cue.}} \\
        b_l &\leftarrow& \mbox{if }  b_l = 0 \mbox{ and } b_r = 0 \mbox{ and } c_{lb} = 1 \mbox{ then} 1 \mbox{ else } b_l \\
        b_r &\leftarrow& \mbox{if }  b_l = 0 \mbox{ and } b_r = 0 \mbox{ and } c_{rb} = 1 \mbox{ then} 1 \mbox{ else } b_r \\
        \multicolumn{3}{l}{\mbox{Sets the left (1) or right (0) direction according to the cue.}} \\
        b_d &\leftarrow& \mbox{if }  c_{lb} = b_l \mbox{ then } 1 \mbox{ elif } c_{rb} = b_r \mbox{ then } 0 \mbox{ else } b_d \\
\end{array}} \end{description}

\subsubsection{Task 3: Valued environment decision} \begin{description} 
  \item[Heuristic] In this case, the previous energy increase has to be compared with the present energy increase, in order to choose the largest. This simply modifies the evaluation of the about-turn trigger, as follows
   \item[Programmatoid solution] \eqline{\begin{array}{rcl}
      b_a &\leftarrow& \mbox{if }  b_1  \mbox{ and } g_{d1} > 0 \mbox{ and } g_e > g_{e1} \mbox{ and } g_e - g_{e1} > g_{d1} \mbox{ else } b_a \\
 \end{array}} \end{description}
...
\subsection{On sequential evaluation}
...
\eqline{\left\{ \begin{array}{rcl} \cdots \\ o_n[t] &=& f_n\left(\cdots i_m[t-1], \cdots, \cdots o_l[t - 1]  \cdots, \cdots o_k[t]  \cdots  \right), k < n \\ \cdots \\ \end{array} \right.}
recursively replacing all right-end side values of $o_k[t]$ by the $ f_n\left(\cdot\right)$ correspondent value, will at the fixed point, yields a instruction set for which the evaluation order is meaningless, since interdependences between output variables has been canceled by substitution.
...
A conditional expression on variables on any numerical type $v_n, n \in \{0, 1\}$  with a binary variable $b_1 \in \{0, 1\}$ writes\footnote{It corresponds to the standard languages conditional expression: \\\centerline{{\tt v = b\_1 ? v\_1 : v\_0},}\\ or the Python construct: \\\centerline{{\tt v = v\_1 if v\_1 else v\_0}.}} : \eqline{\begin{array}{rcl}
...
\subsubsection{On conditional instruction implementation}
...
\\\centerline{\parbox{12cm}{\tt if condition\_1 then instruction\_1 instructions\_2  \\ elif condition\_2 then instructions\_3 \\ … \\ else instructions\_4 endif}}\\
rewrites:
\\\centerline{\parbox{12cm}{\tt
      if condition\_1 then instruction\_1 endif \\
      if condition\_1 then instructions\_2  endif \\
      if not condition\_1 and condition\_2 then instructions\_3 endif \\
      ...\\
      if not condition\_1 and not condition\_2 then instructions\_4 endif}}\\
in words: any conditional instruction decomposes into elementary conditional expressions.

Similarly fixed length loops can be easily expanded, thus can also be introduced as a syntactic sugar in straight-line pieces of codes, e.g. a construct of the form, for a bounded variable value {\tt n\_1, 0 ≤  n\_1 ≤ n\_max}
\\\centerline{\parbox{12cm}{\tt for n from 0 to n\_1 ≤ n\_max do instructions\_1  endfor}}\\
rewrites:
\\\centerline{\parbox{12cm}{\tt if n ≥ 0 then substitute(n =  0, instructions\_1)\\ ...\\if n = n\_1 then substitute(n =  n\_1, instructions\_1)\\ .}}\\
in words: expands the loop in sequence for any possible value, while if {\tt n\_1 > n\_max} a compilation error is to be thrown.
...
\begin{tabular}{cc}
  {\bf Delay on rising front} &  {\bf Time bounded output} \\
  \parbox{0.4\textwidth}{~
\\  - $i_1$ starts the delay
\\  - $i_0$ resets the state and the output
\eqline{\begin{array}{rcl}
             o &\leftarrow& \mbox{ if } v > 1/2 \mbox{ then } 1 \mbox{ else } 0 \\
             v &\leftarrow& \mbox{ if } i_0 = 1 \mbox{ then } 0  \mbox{ else } (1-\gamma) \, v + \gamma \, s \\
             s &\leftarrow& \mbox{ if } s = 0 \mbox{ and } i_1 = 1 \mbox{ then } 1 \\&& \mbox{ elif } s = 1 \mbox{ and } i_0 = 1 \mbox{ then } 0 \\&& \mbox{ else } s \\
  \end{array}}} & \parbox{0.4\textwidth}{~
\\  - $i_1$ starts the bounded time output
\\  - $i_0$ resets the state and the output
\eqline{\begin{array}{rcl}
             o &\leftarrow& \mbox{ if } v < 1/2 \mbox{ and } s = 1 \mbox{ then } 1 \mbox{ else } 0 \\
             v &\leftarrow& \mbox{ if } i_0 = 1 \mbox{ then } 0  \mbox{ else } (1-\gamma) \, v + \gamma \, s \\
             s &\leftarrow& \mbox{ if } s = 0 \mbox{ and } i_1 = 1 \mbox{ then } 1 \\&& \mbox{ elif } s = 1 \mbox{ and } i_0 = 1 \mbox{ then } 0 \\&& \mbox{ else } s \\
  \end{array}}} \\
\end{tabular}
...
At the programmatic level, the bot behavior is directly programmed implementing a {\tt State} in the target programming language: \begin{verbatim}
...
The name {\tt programmaticoid\_…} corresponds to a programmatic implementation in the target programming language, but using only programmatoid constructs. It is used for testing purposes.

In {\tt Python} one can start from the following example:
\begin{center}\href{https://github.com/vthierry/braincraft/blob/master/braincraft/programmatic_task1.py}{\tt programmatic\_task1.py}\end{center}
The \href{https://github.com/vthierry/braincraft/blob/master/braincraft/programmatoid_challenge.py}{{\tt programmatoid\_challenge.py}} source file includes all documented methods and the related API documentation is available here:
...
The {\tt ./out/\$prgm/test.out.wjson} output file format uses a dialect of the \href{https://www.json.org/json-en.html}{JSON} syntax, called {\tt wJSON}, for weak-json as \href{https://line.gitlabpages.inria.fr/aide-group/wjson/}{presented here}.

\subsection{Running at the neuronoid level}
...
\centerline{\includegraphics[width=0.9\textwidth]{./res_archi.png}}
...
\section{An alternative solution with quarter-turn only and no about-turn}

We have proposed half-turn mechanism, because the implementation is relatively simpler, thus clearer, for benchmarking the programmatoid and neuronoid implementations.
However we also can propose a quarter-turn implementation.
...
\\\tab (ii) perform a quarter-turn in the preferred orientation as soon as a wall is closed ahead, or if in the middle of the corridor.
\\- With this navigation mechanism the bot performs either leftward or rightward half-loops, traversing the central corridor.
\\- In fact, the situation is quite complex because the sensors do {\em not} detect any crossing, because the field of view is too narrow. Therefore, predefined delays are to be used, in order to detect tthe middle of the downward corridor.
\item[Internal constants]  \eqline{\begin{array}{rcll}
    \gamma &=& 0.035 & \mbox{\parbox{12cm}{Linear feedback gain in forward mode, maximize stability without oscillation.}} \\
    \alpha    &=& 5.0 & \mbox{\parbox{12cm}{Maximal turn angle, allowing to perform a quater turn in $18$ time steps.}} \\
    \beta_1   &=& 0.8 & \mbox{\parbox{12cm}{Proximity threshold triggering a quater-turn.}} \\
    \beta_0   &=& 0.7 & \mbox{\parbox{12cm}{Proximity threshold ending a quater-turn.}} \\
    \delta _1   &=& 18 & \mbox{\parbox{12cm}{Quarter-turn duration in number of time steps.}} \\
    \delta _0   &=& 8 & \mbox{\parbox{12cm}{Delay between two quarter-turns in number of time steps.}} \\
\end{array}}
\item[Internal variable]  \eqline{\begin{array}{rcll}
  b_d \in \{0_{\mbox{rightward}}, 1_{\mbox{leftward}}\}, &\left.b_d\right|_{t = 0} = 0 & \mbox{Quarter-turn direction, either left (1) or right (0).} \\
  b_t \in \{0_{\mbox{forward}}, 1_{\mbox{turning}}\}, &\left.b_t\right|_{t = 0} = 0 & \mbox{Forward (0) versus turning (1) mode.} \\
  b_{t\bullet} \in \{0, 1\}, \bullet \in \{0, 3\}&\left.b_{t\bullet}\right|_{t = 0} = 0 & \mbox{Turn 0, 1, 2 or 3 mode.} \\
  b_{t30} \in \{0, 1\}&\left.b_{t30}\right|_{t = 0} = 0 & \mbox{Turn 3 waiting delay before turing.} \\
  g_{e\bullet} \in \{0, 1\}, \bullet \in \{0, 1\}&\left.g_{e\bullet}\right|_{t = 0} = 0 & \mbox{Energy after and before turning.} \\
  b_{3w\bullet} \in \{0, 1\}, \bullet \in \{0, 1\}&\left.g_{e\bullet}\right|_{t = 0} = 0 & \mbox{Delay variables for turrn 3 start and stop.} \\
\end{array}}
\item[Implementation] \begin{verbatim}
  ...
  if  g_e0 <  g_e1
    ...
    with the same equations to control the {\tt d\_l} and {\tt d\_r} output variables as before. The behavior is tested in \href{https://raw.githubusercontent.com/vthierry/braincraft/master/braincraft/programmatic_quarter_turn_task1}{programmatic\_quarter\_turn\_task1.py}.
\end{description}

\begin{figure}[H]
  \includegraphics[width=0.5\textwidth,valign=c]{./navigation2.png}
\hfill \begin{minipage}{\dimexpr 0.5\linewidth-\columnsep}
        \caption{Illustrates the quarter turn implementation: it starts when the ahead proximity to a wall exceeds a threshold, and has a predefined duration, using a timer.}
        \label{starting-moves}
      \caption{Illustrates the navigation implementation. The navigation is a loop  made of 4 quarter-turns.
...
\section{Programmatoid package reference}

\subsection{Package and compiler options and outputs}\label{package-options}

\centerline{\small \begin{tabular}{|l|l|l|l|} \hline
{\bf Name}                & {\bf Format}                & {\bf Value} & \\ \hline
 &&&\\\multicolumn{4}{|l|}{{\bf Package meta-data.}} \\
{\tt file} 		& string & {\tt path/name} & \parbox{6cm}{The package source path name, without the '.mpl' extension.}\\
...
{\tt timeout}              & int & 0 & \parbox{6cm}{Maximum number of iterations, no bound if 0.} \\ &&&\\
...
{\tt rate}                    & float & 0 & \parbox{6cm}{Delay in second between two iterations (used for display).} \\ &&&\\
...
\subsection{Programmatoid functions}\label{programmatoid-functions}
