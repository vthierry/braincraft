...
prgm_functions := {
  ...
  Not = ((v) -> H(1 - H_(v))),
  If_b = ((c, b1, b0) ->
    if nargs = 3 then H(H_(b0) - H_(c) - 0.5) + H(H_(b1) + H_(c) - 3/2)
...
prgm_compile := proc(prgm_file :: string)
  ...
  try 
    ...
    if prgm["options"] [network_weights] then  prgm["options"] [all_neuronoid] := true fi:
  ...
  if not convert(prgm["options"][inputs], set) subset indets(prgm["flattened"], name) then
    error cat("Incoherence in variables, the input variables '", prgm["options"][inputs], "' is not a subset of the whole variable set '",  prgm["network"][state], "', it must.") fi:
  if not convert(prgm["options"][outputs], set) subset convert(prgm["network"][lhs], set) then
    error cat("Incoherence in variables, the output variables '", prgm["options"][outputs], "' is not a subset of the left-hand side variable set '", prgm["network"][lhs], "', it must.") fi:
  if convert(prgm["network"][lhs], set) union convert(prgm["options"][inputs], set) <> indets(prgm["flattened"], name) then
    error cat("Incoherence in variables, the left-hand side variables '", prgm["network"][lhs], "' with the input variables '", prgm["options"][inputs], "' does not equals the whole variable set '", prgm["network"][state], "', it must.") fi:
  ...
  prgm["network"][W_out] := Matrix(nops(prgm["options"][outputs]), nops(prgm["network"][lhs]), (o, s) -> if prgm["options"][outputs][o] = prgm["network"][state][s] then 1 else 0 fi):
  prgm["network"][W_in] := Matrix(nops(prgm["network"][lhs]), nops(prgm["options"][inputs]), (s, i)  -> coeff(op(1, op(2, prgm["flattened"][s])), prgm["options"][inputs][i])):
  ...
  if prgm["options"][python] then 
    prgm["python"] := (proc(prgm)
      ...
      d := StringTools[RegSubs]("cg" = "state[\"model\"]", StringTools[RegSubs]("[]][\t\n ]*$" = "", StringTools[RegSubs]("= [[]" = "= ", CodeGeneration[Python]([prgm["network"][W_out], prgm["network"][W_in], prgm["network"][W], prgm["network"][w]], defaulttype=float, output=string)))):
	...
	(if prgm["options"][network_weights] then cat(
	  "\ndef init_state(state)\n",
	  "\t", d, "\n\tinit_state_from_network(state)\n",
	  "\ndef update_state(state)\n\tupdate_state_from_network(state)\n\n") else cat(
	  "\ndef init_state(state)\n",
	  "\tstate.update(dict({",op(map(n->cat("\"",n,"\": 0, "), indets(prgm["flattened"], name)))," \"init\": True}))\n",
	  "\ndef update_state(state)\n",		   
