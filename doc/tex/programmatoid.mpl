...
prgm_compile := proc(prgm_input)
  ...
  # Sets current options after default values, from the proc arguments, then from the prgm_input
     ...
     for o in args[2..nargs] do if type(o, `=`) then prgm_current_options[op(1, o)] := op(2, o) else prgm_current_options[o] := true fi od:
  ...
  (proc(eqs :: list(`=`))
     local eqs0 := [], eqs1 := eqs, max_iterations := 10:
     for t to max_iterations while eqs0 <> eqs1 do
       eqs1 := eqs0: eqs0 := prgm_expand_inner_functions(eval(subs(prgm_functions, eqs1)))
     od
   end)(eqs_input)
end:
...
prgm_expand_inner_functions := proc(eqs::list(`=`))
   ...
   local eq1, eq2 := []:
   eq1 := map(eq -> if type(op(2, eq), function) then
      op(1, eq) = map(proc(a)
         local n:
...
prgm_check_syntax := proc(eqs)
  ...
  if not type(eq, list) then
    error "The  input is not a list (of equations)" fi:
 map(proc(l)
     global eqs: local eq := eqs[l]:
     if not type(eq, `=`) then
       error cat("This line number '", l, "' of the input '", eq, "' is not a an equation") fi:
     if op(1, eq) = 'prgm_options' then
       if i > 1 then
     	  error cat("prgm_options '", eq, "' given at line number '", l, "' of the input, instead of the 1st line") fi:
       if not ((type(op(2, eq), list) or type(op(2, eq), set)) and convert(map(e -> type(e, name) or (type(e, `=`) and type(op(1, e), name)), op(2, eq)), `and`)) then
     	  error cat("prgm_options '", eq, "' given at line number '", l, "' of the input, must be of type `[name = value, ...]` or `{name = value, ...}`") fi:
     fi:  
     if not type(op(1, eq), name) then
         error cat("The left hand side of '", eq, "', line number '", l, "' is not a name") fi:
     if type(op(2, eq), function) and not (op(0, op(2, eq)) in function_names) then
         error cat("The right hand side of '", eq, "', line number '", l, "' is not a known function") fi:
  end, eqs):
end:
...
prgm_new_symbol := proc(prefix:: String)
   ...
   local name := cat(prefix, '_', nops(op(prgm_new_symbol_table))):
   if unassigned(prgm_new_symbol_table[name]) then prgm_new_symbol_table[name] := true: convert(name, name) else prgm_new_symbol(cat(prefix, "_")) fi
 end:
