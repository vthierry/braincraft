   ...
   \includegraphics[width=0.5\textwidth,valign=c]{./delay1.png}
    \hfill \begin{minipage}{\dimexpr 0.5\linewidth-\columnsep}
      \caption{With: \eqline{\begin{array}{rcl}
        ...
        \eqline{T = -\frac{\log(2)}{\log(1 - \gamma)} > 0 \Leftrightarrow \gamma =  1 - e^{-\frac{\log(2)}{T}}.}
   ...
   \includegraphics[width=0.5\textwidth,valign=c]{./astable1.png}
    \hfill \begin{minipage}{\dimexpr 0.5\linewidth-\columnsep}
      \caption{With: \eqline{\begin{array}{rcl}
        ...
        \eqline{T = -\frac{2 \, \log(2)}{\log(1 - \gamma)} > 0 \Leftrightarrow \gamma =  1 - 2 \, e^{-\frac{\log(2)}{T}}.}}
        \label{astable1}
    \end{minipage}
\end{figure}
...
Another track is to consider the programatoid implementation of the maximium operator, given $K$ inputs $i_k \in [-1, 1], k \in \{1, K\}$: \eqline{\begin{array}{rcll}
    ...
    && h_k \defq h\left(\omega \, \left(\sum_{l \in \{1, K\}, l \ne k} H(i_k - i_l) - K - 1/2\right)\right) & \mbox{$b_k$ mollification}\\
\end{array}}
...
With this definition we have constructed  a softmax mechanism, directly based on the previous developmemts, without introducing numerical approximations of other non-linear functions.
...
\subsection{Installation of the setup in four steps}
\begin{enumerate}
...
At the programmatoid level, the system is defined by a set of equations ….

For binary variables $\mbox{{\tt b\_i}} \in \{0, 1\}$, continuous variables $\mbox{{\tt v\_i}} \in [-1, 1]$, while the parameter $\mbox{{\tt T}} > 0$, the following
functions, defined previously, are implemented: \\\centerline{\begin{tabular}{ll}
  {\tt If\_b(b\_1, b'\_1, …, b'\_0)} & Binary expression with an odd number of arguments.\\
  {\tt If\_v(b\_1, v\_1, …, v\_0)} & Valued expression with an odd number of arguments.\\
  {\tt And(v\_1, …)} & Conjunction with a variable number of arguments.\\
  {\tt Or(v\_1, …)} & Disjunction with a variable number of arguments.\\
  {\tt Not(v\_1)} & Negation. \\
  {\tt Latch\_b(b\_1, i\_l)} & Binary memory latch.\\
  {\tt Latch\_v(v\_1, i\_l)} & Valued memory latch.\\
  {\tt Bprod(b\_1, v\_1, …)} & Sum of binary products, with an even number of arguments.\\
  {\tt Bistable(i\_0, i\_1)} & Two inputs bistable latch. \\
  {\tt Bistable(i\_1)} & One input bistable latch. \\
  {\tt Spikeup(i\_1)} & Spike generation on input rising front. \\
  {\tt Delay(i\_1, T)} & Delayed output. The delay {\tt T} is given in number of global clock event.\\
  {\tt Oscillator(i\_1, T)} & Oscillatory output. The period {\tt T} is given in number of global clock event.\\
  {\tt Softmax(v\_1, …, g)} & Mean-max operator, with $g \in [0, 1]$, $g=0$ for average, $g=1$ for maximum \\
\end{tabular}} \\
