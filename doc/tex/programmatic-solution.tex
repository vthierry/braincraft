...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.4\textwidth]{./explog.jpg}}
\caption{Representation of the exp-log function for $K = 2$, $p[2] = 1/2$, with $p[1] \in [0, 1]$ and $\mu \in \{1, 2, 100\}$. With small $\mu$ it is closed to linear average, whereas for $\mu =100$ it is close to $\max(x, 1/2)$.}
\label{explog}
\end{figure}
\end{description}
...
\paragraph{Energy measurement}\begin{description}
 \item[Motivation] The energy increase accumulation has to be pre-processed, since used in some task.
...
\end{array}}
Here instantaneous energy increase is $(g_{e} - g_{e1})$, 
\item[Implementation] \eqline{\begin{array}{l}
    ...
    \mbox{Otherwise $g_{e} \le g_{e1}$, thus cumulating energy increase stops, and $g_{c1}$ is memorized.} \\
  \end{array}}
...
\end{array}}
  ...
  \eqline{H(u) \, v \simeq h(v/\upsilon + \upsilon \, h(\upsilon \, u) - \upsilon)}
  where $h(\cdot)$ is the sigmoid function, and $\upsilon$ a sufficiently large member.
\end{description}
...
\subsubsection{Direction choices}
...
\subsubsection{Task 1: Simple decision} \begin{description}
  ...
  \item[Heuristic] If the energy is too low, thus looping in the wrong direction, the direction is changed once. \\
   ...
   \end{array}} where: \eqline{\begin{array}{rcll}
      ...
      \nu &\simeq& b \, c/s = 3/20& \mbox{\parbox{8cm}{Energy consumption threshold if no source on the path.}}\\
  \end{array}}
\end{description}
