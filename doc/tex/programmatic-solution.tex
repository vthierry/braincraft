...
\section*{In progress} \begin{itemize}

\item Creer un doc/index.html manuel

\item Commener le compilateur qui
...
\item Analyse situation de recurrence
  
\item Neuronoid approximation of the explog
...
Derivations are available \href{https://raw.githubusercontent.com/vthierry/braincraft/master/doc/tex/mollificatio.mpl.out.txt}{{\tt Maple} code}.
...
Derivations are available \href{https://raw.githubusercontent.com/vthierry/braincraft/master/doc/tex/linearapproximation.mpl.out.txt}{{\tt Maple} code}.
...
\eqline{\begin{array}{rcl}
    ...
    o(t) &\leftarrow& \omega' \, (h(i(t)/\omega' -  \omega \, i_l(t)) + h(o(t - 1) (t)/\omega' -  \omega \, (1 - i_l(t))) \\
    &=& \left. O\left(\omega' \, e^{-4\,\omega}\right) + \omega' \, h(o(t - 1) (t)/\omega') \right|_{i_l(t) = 1} \\
\end{array}}

The former equation is exact, so totaly stable entirely stable during iterations.

For the second equation, memorizing $o(0) \in \{0, 1 \}$, thus with $i_l(t) = 1$, for
...
\eqline{\begin{array}{rcl}
  \epsilon(t) &=& \beta_{i(t),o(0)} + h( \omega \, (\epsilon(t-1) - 1/2)), \\ \mbox{ with } \beta_{i(t),o(0)} &\defq&
  \begin{array}{|c|c|c|} \hline
                       & o(0) = 0                                                                             & o(0) = 1 \\ \hline
           i(t) = 0 & h(-3/2 \, \omega) &  - h(-3/2 \, \omega) \\ \hline
           i(t) = 1 & h(-1/2 \, \omega) & - h(-1/2 \, \omega)  \\ \hline
    \end{array} \\ \end{array}}

AHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHh

 %\subsection{Monostable binary value (delay)}
%avec l = l/2 + h() si neuronoid pour avoir v fini
%\subsection{Bistable binary value (divider by 2)}
%Then: emporizing mechanisms, oscillators and sequence generator, sleep sort mechanism, 
%\subsection{Bistable astable value (oscillator)}
...
\section{Comparing {\tt tanh} and {\tt arctan} non linearities}

We have $h(x) = \frac{1 + \tanh(2 \, x)}{2}$ thus based on the {\tt tanh} non linearity.
We also could have choose:
\eqline{\bar{h}(x)  = \frac{1}{2} + \frac{\mbox{arctan}\left(\pi  \, x\right)}{\pi},}
we have another normalized sigmoid:
\eqline{\bar{h}(-\infty) = 0, \bar{h}(0) = 1/2, \bar{h}'(0) = 1, \bar{h}(+\infty) = 1, \bar{h}(x) = 1 - \bar{h}(-x)}
with:
\eqline{h(x) > \bar{h}(x), x > 0, |h(x)-\bar{h}(x)|_{{\cal L}_1} = +\infty, |h(x)-\bar{h}(x)|_{{\cal L}_0} \simeq 0.082,}

However, it is less efficient to approximate the step function, as visible in the Figure~\ref{sigmoidatan}.
\begin{figure}[htbp]
  \includegraphics[width=0.5\textwidth,valign=c]{./sigmoidatan.jpg}
    \hfill \begin{minipage}{\dimexpr 0.5\linewidth-\columnsep}
      \caption{Comparision between  the {\tt tanh} and {\tt arctan} non linearities, showing that the former is sharper. The maximal difference is about $8\%$ at $x \simeq 0.85$.}
        \label{sigmoidatan}
    \end{minipage}
\end{figure}
