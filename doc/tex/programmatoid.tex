...
\subsubsection{Energy measurement}\begin{description}
 \item[Motivation] The energy increase accumulation is to be preprocessed, since used in some task.
...
\item[Implementation] \eqline{\begin{array}{l}
    \mbox{Cumulative energy increase starts, saving last increase in $g_{c2}$.} \\
    ...
    \mbox{Cumulative energy increase continues.} \\
    ...
    \mbox{Otherwise $g_{e} < g_{e1}$, thus cumulative energy increase stops, and $g_{c1}$ is memorized.} \\
    g_s \defq g_{e} < g_{e1}  \mbox{ and } g_{e1} > g_{e2} \mbox{ indicates that cumulative energy increase has just stopped.} \\
    g_n \defq g_{e} < g_{e1}  \mbox{ and } g_{e1} < g_{e2} \mbox{ indicates that cumulative energy increase did stop before.} \\
  \end{array}}
...
\end{array}}  in words: we execute the quarter-turn until another wall is detected\footnote{The identities:
...
A conditional expression on variables on any numerical type $v_n, n \in \{0, 1\}$  with a binary variable $b_1 \in \{0, 1\}$ writes: \eqline{\begin{array}{rcl}
    ...
    &=& H(v_1 + b_1 - 3/2) + H(v_0 + (1 - b_1) - 3/2) \\
    &=& H(v_1 + b_1 - 3/2) + H(v_0 - b_1 - 1/2) \\
    &=& H(H(v_1 + b_1 - 3/2) + H(v_0 - b_1 - 1/2)) \\
\end{array}}
...
It has been noticed that, except for numerical comparisons, all arguments $x$ of the step function $H(\cdot)$ verify $|x| \ge 1/2$, and the related error is negligible as soon as, say, $\omega \ge 10$:
...
\begin{figure}[H]
  \includegraphics[width=0.5\textwidth,valign=c]{./expneuronoid.jpg}
    \hfill \begin{minipage}{\dimexpr 0.5\linewidth-\columnsep}
      \caption{Approximation in the $[-1, 1]$ interval of the exponential function, in blue, by a sigmoid combination, in green: \eqline{0.182 + 2.34 \,h(x - 1) + 1.55 \, h(x/2): } yielding a ${\cal L}_1$ error of $0.01$, the error being drawn in red.}
        \label{expneuronoid}
    \end{minipage}
\end{figure}
...
\begin{figure}[H]
  \includegraphics[width=0.5\textwidth,valign=c]{./logneuronoid.jpg}
    \hfill \begin{minipage}{\dimexpr 0.5\linewidth-\columnsep}
      \caption{Approximation in the $[1, e]$ interval of the logarithm $\log(x)$ function, in blue, by a sigmoid combination, in green: \eqline{-5.10 + 2.60 \,h(x/2) + 4.70 \, h(x/10): } yielding a ${\cal L}_1$ error of $0.001$, the error being drawn in red.}
        \label{logneuronoid}
    \end{minipage}
\end{figure}
...
\subsection{Neuronoid approximation of valued product.}
...
with:
\eqline{b_k \defq And_{l \in \{1, k-1\}} H(i_k >  i_l) \mbox{ and } And_{l \in \{k+1, K\}} H(i_k \ge i_l),}
...
For $k_0$, $And_{l \in \{k_0+1, K\}} H(i_{k_0} \ge i_l) = 1$ because it is a maximum and $And_{l \in \{1, k_0-1\}} H(i_{k_0} >  i_l) =1$ because no previous maximum is encountered, by definition. Then for subsequent values $k_1 = \max_k(i_k)$, because there is $k_0 < k_1$ with $i_{k_0} = i_{k_1}$, then $And_{l \in \{1, k_1-1\}} H(i_{k_1} >  i_l) = 0$, so that we only have one occurrence of $i_k$ which is selected.}, thus with a form similar to the average operator $\mbox{mean}_k(i_k) = 1/K \, \sum_k b_k \, i_k$.

We easily derive:
\eqline{\begin{array}{rcl}
    b_k &\defq& And_{l \in \{1, k-1\}} H(i_k >  i_l) \mbox{ and } And_{l \in \{k+1, K\}} H(i_k \ge i_l) \\
    \multicolumn{3}{l}{\mbox{while, expanding the $And$ operator}}\\
    &=&  H\left(\sum_{l \in \{1, k-1\}}  H(i_k >  i_l) + \sum_{l \in \{k+1, K\}} H(i_k \ge i_l) - K + 3/2\right). \\
\end{array}}
...
\section{Comparing {\tt tanh} with other non linearity}
...
subs({ ## Fixed parameters constant values
    T = 10,                                             ### The delay
    constantName = constantValue  ### Any other …
},
[  ## The package options 
...
The compilation is parameterized by a set of options given in appendix~\ref{programmatoid-options} and is defined using a set of functions given in appendix~\ref{programmatoid-functions} corresponding to all functionnalities defined in this document. 
\end{verbatim}
...
\subsection{Programmatoid package meta-data and options}\label{programmatoid-options}

Available options: \\\begin{tabular}{|l|l|l|l|} \hline
{\bf Name}                & {\bf Format}                & {\bf Value} & \\ \hline
 &&&\\\multicolumn{4}{|l|}{{\bf Package meta-data.}} \\
{\tt name} & string & & \parbox{8cm}{The package name, deduced from the source file name, which is of the form {\tt name.mpl}.}\\
{\tt file} & string & \parbox{2cm}{set during \\ compilation} & \parbox{8cm}{The package source path name, which is of the form {\tt path/name.mpl}.}\\
{\tt description} & string & & \parbox{8cm}{A one or two lines description. }\\
{\tt homepage} & URL & & \parbox{8cm}{A link to the package, e.g., a git page.}\\
{\tt version} & date & \parbox{2cm}{set during \\ compilation}  & \parbox{8cm}{The package version, given as the date of compilation.}\\
{\tt license} & string & CECILL-C & \parbox{8cm}{The share licence. }\\
{\tt author} & Name $<$email$>$ & & \parbox{8cm}{The name and email of the contact person.} \\
 &&&\\ \multicolumn{4}{|l|}{{\bf Compiler parameters.}} \\
{\tt omega}                    & large positive float & 1000 & \parbox{8cm}{The $\omega$ used for both {\tt Id()} and {\tt H()} functions mollification.} \\ &&&\\
{\tt all\_neuronoid}       & Boolean                   &  false & \parbox{8cm}{Whether {\tt Id()}  and {\tt H()} functions are mollified, remaining with {\tt h()} function only.} \\ &&&\\
{\tt network\_weights}  & Boolean                   &  false & \parbox{8cm}{Whether not a code, but the weights of a recurrent network with {\tt h()} as non-linearity, is generated.} \\ &&&\\
{\tt python}                & Boolean    &  true    & \parbox{8cm}{If defined, generates a Python piece of code of name {\tt update\_state} with the flattened code.} \\ &&&\\
{\tt inputs}                 & List of names          &  deduced & \parbox{8cm}{The list of input variables, if it does not correspond to the right-hand side expression variables, not present in any left-hand side equation, in the initial equation order.} \\ &&&\\
{\tt outputs}               & List of names          &  deduced & \parbox{8cm}{The list of output variables, if it does not correspond the left-hand side variables, not present in right-hand side expressions, in the initial equation order.} \\ &&&\\
\hline \end{tabular}

\newpage \subsection{Programmatoid compilation output}\label{programmatoid-compilation}
\begin{tabular}{|l|l|l|} \hline
{\bf Name}                   & {\bf Type}               & \\ \hline &&\\
{\tt options}                  & string                      & \parbox{8cm}{The compilation options, given in the code first line.} \\
{\tt options[name]}      & string                      & \parbox{8cm}{The programmatoid name.} \\
{\tt options[file]}          & string                      & \parbox{8cm}{The programmatoid source path name.} \\
{\tt options[version]}   & date                         & \parbox{8cm}{The compilation version.} \\
{\tt source}                   & string                       & \parbox{8cm}{The input source file content, including the comments.} \\
{\tt input}                     & list({`=`,function})   & \parbox{8cm}{The parsed input source, without the options.} \\
{\tt expanded}             & list({`=`,function})   & \parbox{8cm}{The input, with known functions expanded.} \\
{\tt flattened}             & list({`=`,function})    & \parbox{8cm}{The expanded input, with all inner functions generated by new equations.} \\
{\tt python}                 & string                      & \parbox{8cm}{The flattened input, as a {\tt def name(state)} function, saved in a {\tt name.py} file.}\\
{\tt model[state]}        & list(name)              & \parbox{8cm}{The list of all state variables of the programmatoid.}\\
{\tt model[lhs]}          & list(name)              & \parbox{8cm}{The list of all left-hand side variables of the programmatoid.}\\
{\tt model[rhs\_f]}      & list(name)              & \parbox{8cm}{The list of all rigth-hand side non-linearity functions of the programmatoid.}\\
{\tt model[the\_f]}      & list(name)              & \parbox{8cm}{The rigth-hand side non-linearity function, if unique.}\\
{\tt model[W\_in]}      & matrix                    & \parbox{8cm}{The binary matrix that maps input vector on the state variables vector.}\\
{\tt model[W\_out]}     & matrix                    & \parbox{8cm}{The binary matrix that mapsthe state variables vector onto the output vector.}\\
{\tt model[W]}             & matrix                    & \parbox{8cm}{The recurrent weights matrix.}\\
{\tt model[w]}             & matrix                    & \parbox{8cm}{The recurrent weights offset, or bias.}\\
{\tt error}                     & string                     & \parbox{8cm}{The error message, if an error occurs during input source analysis.} \\
\hline \end{tabular}
