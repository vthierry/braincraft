...
\eqline{\begin{array}{|ll|l|}
  ...
  p_l \in ]0_{\mbox{no-wall}}, 1_{\mbox{wall-hit}}] && \mbox{\parbox{5cm}{~\\Leftward proximity, at +30° on the left.\\}} \\\hline 
  p_r \in ]0_{\mbox{no-wall}}, 1_{\mbox{wall-hit}}] && \mbox{\parbox{5cm}{~\\Rightward proximity, at -30° on the  right.\\}} \\\hline 
  p_a \in ]0_{\mbox{no-wall}}, 1_{\mbox{wall-hit}}] && \mbox{\parbox{5cm}{~\\Ahead proximity, at 0°.\\}} \\\hline 
...
\\ \tab - we consider three leftward, ahead, and rightward external sensors only,
...
\\- A simple sum or average could also be used, thus using directly a linear combination of the input in afferent units. \\
  \eqline{p_\dagger \leftarrow 1 / 32 \, \sum_{k \in K_\dagger} p[k], \dagger \in \{l_{\mbox{left}}, r_{\mbox{right}},\}}
...
\item[Implementation] It uses three variables
\eqline{\begin{array}{lll}
  g_{e1} \in [0, 1], &\left.g_{e1}\right|_{t = 0} = 0 & \mbox{Energy before turning in the feeding corridor.} \\
  g_{e0} \in [0, 1], &\left.g_{e0}\right|_{t = 0} = 0 & \mbox{Energy when turning out from the feeding corridor.} \\
  g_{ed} \in [0, 1], &\left.g_{ed}\right|_{t = 0} = 0 & \mbox{Previous feeding corridor energy difference, positive if an energy source.} \\
\end{array}}
...
\\\tab (ii) perform a quarter-turn in the preferred orientation as soon as a wall is closed ahead.
\\With this navigation mechanism the bot performs either leftward or rightward half-loops, traversing the central corridor.
In fact, the situation is quite complex because the sensors do {\em not} detect any crossing, because the field of view is too narrow. Therefore, proximity to the wall, and predefined delays are to be used.
...
\item[Internal variables] \eqline{\begin{array}{lll}
  b_d \in \{0_{\mbox{rightward}}, 1_{\mbox{leftward}}\}, &\left.b_d\right|_{t = 0} = 0 & \mbox{Preferred quarter-turn direction.} \\
  b_t \in \{0_{\mbox{forward}}, 1_{\mbox{turning}}\}, &\left.b_t\right|_{t = 0} = 0 & \mbox{Forward versus turning mode.} \\
  b_{t\bullet} \in \{0,  1\}, \bullet \in \{0, 1, 2, 3_0, 3\} &\left.b_{t\bullet}\right|_{t = 0} = 0 & \mbox{Turn0, Turn1, Turn2, Turn3-delay, and Turn3 modes respectively.} \\
\end{array}}
...
\item[Programmatoid solution]:\eqline{\begin{array}{rccc}
    ...
    &                 & \mbox{\small \begin{tabular}{c}linear correction to \\ maintain direction ahead\end{tabular}} & \mbox{\small \begin{tabular}{c}quarter-turn \\ left or right\end{tabular}} \end{array}}
 while the bot turning state implementation writes:
\eqline{\begin{array}{rcll}
\end{array}}
\end{description}

The $\mbox{Delay\_1}(q_d, q_t, \delta)$ constructs, precisely defined in the sequel, stands for a time bounded output:\\
\tab \tab $q_d \leftarrow 1$ when $q_t$ raises to $1$ during $\delta$ time steps, before being reset to $q_d \leftarrow 0$.

Given this controller the design reduces to navigation direction choice, i.e., control the $b_d$ variable.
...
\subsubsection{Task 1: Simple decision} \begin{description}
  ...
  \item[Heuristic] If the energy is too low, thus looping in the wrong direction, the direction is changed once. \\
    At start $b_d = 0$. Then, if the energy is too low it changes once to $b_d = 1$.
\item[Programmatoid solution] \eqline{\begin{array}{l}
          b_d \leftarrow \mbox{if } b_d = 1  \mbox{ or } \eta > g_e \mbox{ then } 1 \mbox{ else } 0 \\
      \end{array}}
...
\subsubsection{Task 1b: Simple decision but varying environment} \begin{description}
  ...
  \item[Heuristic]~
    ...
    \item[Programmatoid solution] \eqline{\begin{array}{rcl}
        ...
        b_d &\leftarrow& \mbox{if }  \eta > g_e  \mbox{ and } g_c = 0 \mbox{ then } 1 - b_d \mbox{ else } b_d \\
...
\subsubsection{Task 2: Cued environment decision} \begin{description}
  ...
  \item[Heuristic] Detect and store the first color blob, and choose to turn in the direction it appears again. \\ The detection is reset when the energy decreases.
    ...
    \item[Programmatoid solution] \eqline{\begin{array}{rcl}
        ...
        b_d &\leftarrow& \mbox{if }  c_{lb} = c_{cb} \mbox{ or }  c_{lr} = c_{cr} \mbox{ then } 0 \mbox{ elif } c_{rb} = c_{cb} \mbox{ or }  c_{rr} = c_{cr} \mbox{ then } 1 \mbox{ else } b_d \\
...
\subsubsection{Task 3: Valued environment decision} \begin{description}
   ...
   \end{array}}
  \item[Programmatoid solution] \eqline{\begin{array}{rcl}
      b_d &\leftarrow& \mbox{if } b_{id} \mbox{ and }  b_{i} \mbox{ then } 1 - b_d \mbox{ else } b_d \\
...
class State:
  ...
  def init(self):
  # Updates, at each step, the data from input and sets the output value.\end{verbatim}

In {\tt Python} one can start from the followinh example:
