...
\subsubsection{Output normalization}\begin{description}
  ...
  \item[Implementation] This can be considered as a minimal output linear unit: \eqline{
...
We name ``programmatoid computation'' the conception of an input-output \href{https://en.wikipedia.org/wiki/Straight-line_program}{straight-line program} implementing an operator on either binary values $\in \{0, 1\}$ or continuous normalized\footnote{An alternative would have be to consider continuous normalized values $\in [-1, 1]$, with the sign function $Sg(x) \defq \mbox{if } x > 0 \mbox{ then } 1 \mbox{ elif } x < 0 \mbox{ then } -1 \mbox{ else } 0$, and its mollification using $\mbox{tanh}(\cdot)$, which would have yields to same general results, with somehow heavier derivations, as the reader can verify. Implementing signed variable is obvious in a programmatoid environment:
\eqline{s \defq s_+ - s_-, s_+ \defq \mbox{if } s  > 0 \mbox{ then } s\mbox{ else } 0, s_-\defq \mbox{if } s  < 0 \mbox{ then } -s \mbox{ else } 0,}
thus manipulating signed variables in a environment with only positive variables.} values $\in [0, 1]$, which evolution is defined using an LN-system with linear units or the step-function $H(\cdot)$ as non-linearity, this writing:
\eqline{o_n[t] = \left\{ \begin{array}{r} H(z_n[t]) \\ Id(z_n[t)] \\ \end{array} \right., z_n[t] \defq \sum_{m\in\{1,M\}} w_{nm} \, i_m[t-1] + w_{n0}, n \in \{1, N\}}
where $Id(\cdot)$ stands for the identity function. Here
\\\tab - $i_m[t]$ is the $m$-th input at a discrete $t$, and
...
A generalized programmatoid may includes other non linearities; this will not used here.
...
\eqline{\begin{array}{rcll}
  \multicolumn{2}{l}{\tau \frac{\partial v_i}{\partial t}(t) + v_i(t) = z_i(t),} &
  \multicolumn{2}{l}{z_i(t) \defq h\left(\sum_j \, w_{ij} \, v_j(t) + w_{i0}\right),} \\ \\
  h(x) &\defq& \frac{1}{1+e^{-4\,x}} \\
         &=& \frac{1+ \mbox{tanh}(2 \, x)}{2} & \mbox{(tanh linear relation)}\\
         &=& h(x_0) + \mbox{sech}(2 \, x_0)^2 \, (x - x_0) + O((x - x_0)^2), & \mbox{(local 1st order behavior)}\\
         &=& 1 - e^{-4 \,x} + O(e^{-8 \, x)}) & \mbox{(behavior toward infinity)}\\
\end{array}}
where $v_i$ stands for the membrane potential, so that:
...
We call ``neuronoid computation'' the conception of an input-output transform based on feed-forward and recurrent combination of only neuronoids:
\eqline{o_n[t] = h(z_n[t), z_n[t] \defq \sum_{m\in\{1,M\}} w_{nm} \, i_m[t-1] + w_{n0}, n \in \{1, N\}}
with the same notations as for programmatoid computations.
...
It must be noted that although continous variables stands in $[0, 1]$ because of negative weights in the linear combination of input, intermediate values may be negative.
...
The implementation as neuronoid, thus allows one to implement more formula than only using programmatoid.
