...
\begin{figure}[H]
  \includegraphics[width=0.9\textwidth,valign=c]{./starting-moves.png}
  \caption{Typical values of the maximal angle lateral sensors values}
...
\item[Programmatoid solution]:\eqline{\begin{array}{rcccc}
    ...
    t_l &\leftarrow& \multicolumn{3}{l}{\mbox{if } q_p = 0 \mbox { and } p_a > \beta \mbox{ then } 1 \mbox{ else } 0} \\
    t_r &\leftarrow& \multicolumn{3}{l}{\mbox{if } q_p = 1 \mbox { and } p_a > \beta \mbox{ then } 1 \mbox{ else } 0} \\
\end{array}}  where: \eqline{\begin{array}{rcll}
    \gamma &=& 0.0 & \mbox{\parbox{8cm}{Experimentally adjusted to maximize stability without oscillation.}} \\
    \alpha    &=& 5.0 & \mbox{\parbox{8cm}{Maximal turn angle, allowing to perform a quatuer turn in $18$ time steps.}} \\
...
{\tt delay}                 & float & 0 & \parbox{6cm}{delay in second bewtween two iterations.} \\ &&&\\
